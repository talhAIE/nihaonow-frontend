...
export default function FeedbackPopup({ isOpen, onClose, scores, transcription }: FeedbackPopupProps) {
  ...
  const copy = {
    ar: {
      title: "تغذية راجعة",
      subtitle: "نتائج التسجيل الصوتي",
      recorded: "ما تم تسجيله:",
  ...
  const feedbackItems = scores ? [
    { label: t.pronunciation, value: scores.pronunciation, icon: <UserRound className="w-5 h-5 sm:w-6 sm:h-6 text-blue-500" /> },
    { label: t.accuracy, value: scores.accuracy, icon: <Target className="w-5 h-5 sm:w-6 sm:h-6 text-red-500" /> },
    { label: t.fluency, value: scores.fluency, icon: <Zap className="w-5 h-5 sm:w-6 sm:h-6 text-yellow-500 fill-yellow-500" /> },
    { label: t.completeness, value: scores.completeness, icon: <CheckSquare className="w-5 h-5 sm:w-6 sm:h-6 text-green-500" /> },
  ] : [];
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4 overflow-y-auto" onClick={onClose}>
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-lg relative animate-in fade-in zoom-in duration-200 my-auto max-h-[95vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        {/* Content */}
        <div className={`p-4 sm:p-6 md:p-8 ${isAr ? "font-almarai" : "font-nunito"}`} dir={dir} lang={isAr ? "ar" : "en"}>
          {/* Header */}
          <div className="text-center mb-4 sm:mb-6 md:mb-8">
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-1 sm:mb-2">{t.title}</h2>
          ...
          {transcription && (
            <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl sm:rounded-2xl border border-blue-100">
              <p className="text-xs text-blue-600 font-medium mb-1 sm:mb-2 text-center">{t.recorded}</p>
          ...
          {scores && (
            <div className="mb-4 sm:mb-6 p-4 sm:p-6 bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl sm:rounded-2xl border-2 border-green-200">
              <div className="text-center">
                <p className="text-xs sm:text-sm text-green-700 font-medium mb-1 sm:mb-2">{t.total}</p>
          ...
          {scores && (
            <div className="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
              {feedbackItems.map((item, index) => (
                <div key={index} className={`flex items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-lg sm:rounded-xl hover:bg-gray-100 transition-colors ${isAr ? "flex-row-reverse" : "flex-row"}`}>
