...
import navLogoPng from "@/assets/landingpage/zayd-mascot-orange.png";
  ...
  return (
    <nav className={`w-full md:py-4 py-8 shadow-sm sticky top-0 bg-white z-[9999] ${isAr ? 'font-almarai' : 'font-nunito'}`} dir={isAr ? "rtl" : "ltr"}>
      <div className="flex items-center w-full px-4 relative">
        ...
        {/* Centered green background menu - absolutely centered on page */}
        <ul className="hidden lg:flex absolute left-1/2 -translate-x-1/2 gap-6 bg-[#E4F2E7] px-6 py-2 rounded-full text-gray-700 font-medium whitespace-nowrap z-10">
          {menuItems.map((item) => {
        ...
        <div className="hidden lg:flex gap-2 items-center ms-auto">
          <button
            onClick={() => setLanguage(language === "ar" ? "en" : "ar")}
            className="text-sm text-gray-700 hover:text-[#35AB4E] transition-colors px-2 py-1"
          >
          ...
          <a href="/login">
            <button
              className="bg-white w-28 text-[#4B5563] rounded-xl px-5 py-2 h-10 text-sm font-bold transition-all hover:bg-gray-50 hover:text-[#35AB4E] active:scale-95 flex items-center justify-center whitespace-nowrap"
              style={{
          ...
          <a href="/register">
            <button
              className="text-white w-28 text-sm font-bold rounded-xl flex items-center justify-center transition-all hover:brightness-110 active:scale-95 px-5 h-10 whitespace-nowrap"
              style={{
                background: "#35AB4E",
