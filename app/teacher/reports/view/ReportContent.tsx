...
export default function StudentReportContent() {
    ...
    const effectiveIsAr = isAr;

    const t = effectiveIsAr
        ? {
            loading: "جاري تحميل التقرير...",
            errorLoad: "حدث خطأ أثناء تحميل بيانات التقرير.",
            notFound: "لم يتم العثور على بيانات الطالب",
            back: "عودة لقائمة الطلاب",
            download: "تحميل",
            downloading: "جاري التحميل...",
            downloadPdfTitle: "تحميل PDF",
            totalPoints: "مجموع النقاط",
            usageTime: "وقت الاستخدام",
            longestStreak: "🔥 أطول سلسلة",
            currentStreak: "⭐ سلسلة انجازات حالية",
            day: "يوم",
            score: "نتيجة",
            accuracy: "رشاقة",
            fluency: "طول",
            totalPointsLabel: "مجموع النقاط",
            lessonProgress: "تقدم المحاضرات",
            remainingLessons: "محاضرات متبقية",
            completedLessons: "محاضرات مكتملة",
            achievements: "الإنجازات",
            noAchievements: "لا توجد إنجازات حتى الآن",
            achievementFallback: "إنجاز",
            certificates: "شهادات",
            noCertificates: "لا توجد شهادات حتى الآن",
            certificateFallback: "شهادة",
        }
        : {
            loading: "Loading report...",
            errorLoad: "Failed to load report data.",
    ...
    if (isLoading) {
        return (
            <div className={`flex flex-col items-center justify-center min-h-screen text-[#35AB4E] font-bold gap-4 ${isAr ? "font-almarai" : "font-nunito"}`}>
    ...
    if (!studentId || error || !report) {
        return (
            <div className={`flex flex-col items-center justify-center min-h-screen text-slate-500 font-bold space-y-4 ${isAr ? "font-almarai" : "font-nunito"}`} dir={isAr ? "rtl" : "ltr"}>
    ...
    return (
        <div className={`min-h-screen ${isAr ? "font-almarai" : "font-nunito"}`}>
            <div className={`flex ${isAr ? "justify-start" : "justify-end"} max-w-4xl mx-auto px-4 pt-6`}>
                <AuthLanguageToggle activeBgClass="bg-[#35AB4E]" activeShadowClass="shadow-[0_2px_0_0_#20672F]" />
            </div>
            <div
                className={`report-container ${isDownloading ? 'is-exporting' : ''} ${effectiveIsAr ? "rtl" : "ltr"}`}
                dir={effectiveIsAr ? "rtl" : "ltr"}
                ref={reportRef}
            >
            <div data-page-section="page1">
                ...
                <div className="report-header">
                    <div className="report-actions">
                        <button
                            ...
                            aria-label={t.downloadPdfTitle}
                            title={t.downloadPdfTitle}
                        >
                            {isDownloading ? (
                                <>
                                    <span className="report-download-text">{t.downloading}</span>
                                    <Loader2 className="w-4 h-4 animate-spin" />
                                </>
                            ) : (
                                <>
                                    <span className="report-download-text">{t.download}</span>
                    ...
                    <div className="report-header-row">
                        <div className="header-top-left">
                            <svg className={effectiveIsAr ? "ml-2" : "mr-2"} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="streaks-grid">
                    <div className="max-streak-card">
                        <div className="card-label">{t.longestStreak}</div>
                        <div className="streak-value-row">
                            <div className="big-number">{report.longestStreak || 0}</div>
                            <div className="small-text">{t.day}</div>
                        </div>
                    </div>

                    <div className="streak-card">
                        <div className="card-label">{t.currentStreak}</div>
                        <div className="streak-value-row">
                ...
                <div className="white-card">
                    ...
                    <div className="stats-grid">
                        ...
                        <div className="stats-row">
                            <div className="stat-label">{t.fluency}</div>
                            <div className="stat-value">{report.averageFluency || 0}</div>
                        </div>
                        <div className="stats-row">
                            <div className="stat-label points-label">{t.totalPointsLabel}</div>
                ...
                <div className="white-card">
                    ...
                    <div className="interaction-grid">
                        <div className="interaction-box pink-box">
                            ...
                            <div className="interaction-label">{t.remainingLessons}</div>
                        </div>
                        <div className="interaction-box green-box">
                            <div className="interaction-value green-text">{completedLectures}</div>
                            <div className="interaction-label">{t.completedLessons}</div>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div className="achievements-grid">
                        {achievements.map((achievement: any, idx: number) => (
                            <div key={idx} className="achievement-badge">
                                ...
                                <div className="achievement-name">{achievement.name || t.achievementFallback}</div>
                            </div>
                        ))}
                    </div>
                ) : (
                    <div className="text-center text-[#9ca3af] py-8">{t.noAchievements}</div>
                )}
            </div>
                    ...
                    <div className="certificates-list">
                        {certificates.map((cert: any, idx: number) => (
                            <div key={idx} className="certificate-card">
                                ...
                                <div className="certificate-title">{cert.name || t.certificateFallback}</div>
                            </div>
                        ))}
                    </div>
                ) : (
                    <div className="text-center text-[#9ca3af] py-8">{t.noCertificates}</div>
                )}
            </div>
        </div>
        </div>
